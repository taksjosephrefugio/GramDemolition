...
get_project_dir_location() {
    ...
    while [[ $(basename "$(pwd)") != "GramDemolition" ]]; do pushd .. > /dev/null || exit; done
    retval=$(pwd)
    pushd -0 >/dev/null || exit     # Return to stack bottom
    dirs -c
    echo "$retval"
}
...
add_env_vars() {
    ...
    if [[ -n $(tail -c 1 "$BASHRC") ]]; then echo >> "$BASHRC"; fi

    # Insert export command to .bashrc
    { echo; echo "$START_DELIMETER"; echo "$COMMAND"; echo "$END_DELIMETER"; } >> "$BASHRC"
}
